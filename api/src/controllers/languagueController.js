...
const deleteLanguage = async (req, res) => {
  ...
  try {
    ...
    if (language) {
      ...
      await language.save();
      return res.status(200).json({ message: `The language ${language.language_name} has been deleted.` });
    }
...
const putEditorial = async (req, res) => {
  ...
  if (language_id === undefined) return res.status(400).json({ message: "Id is required." });
  ...
  try {
    ...
    if (language) {
      ...
      await language.save();
      return res.status(200).json({ message: `The language has been updated.` });
    }
    res.status(404).json({ message: `There is no language with id ${language_id}.` });
  } catch ({ message }) {
    res.status(500).json({ error: message });
  }
}
