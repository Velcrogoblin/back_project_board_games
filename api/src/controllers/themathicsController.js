...
const deleteThematic = async (req, res) => {
  ...
  try {
    ...
    if (thematic) {
      ...
      await thematic.save();
      return res.status(200).json({ message: `The thematic ${thematic.thematic_name} has been deleted.` });
    }
...
const putThematic = async (req, res) => {
  ...
  if (thematic_id === undefined) return res.status(400).json({ message: "Id is required." });
  ...
  try {
    ...
    if (thematic) {
      ...
      await thematic.save();
      return res.status(200).json({ message: `The thematic has been updated.` });
    }
    res.status(404).json({ message: `There is no thematic with id ${thematic_id}.` });
  } catch ({ message }) {
    res.status(500).json({ error: message });
  }
}
